...
- 🤖 AI 总结：支持一键生成本周/本月工作总结，兼容任意 OpenAI API 规范模型服务。
...
| 字段 | 必填 | 说明 |
|------|------|------|
| `summaryType` | 是 | `week` 或 `month` |
| `dateAnchor` | 否 | 锚点日期（不传默认今天），用于计算周/月范围 |
| `model` | 否 | 临时覆盖默认模型 |
...
```json
{
  ...
  "maxTokens": 4000,
  "timeoutMs": 20000,
  "systemPrompt": "你是工作日志分析助手..."
}
```

说明：
- 配置与总结结果写入 SQLite（`ai_model_config`、`ai_summary_record`）。
- AI 配置完全由数据库管理，不依赖 `.env` 中的模型参数。
...
- 头部新增“本周总结/本月总结”按钮，点击后调用 AI 总结接口并在弹窗展示结果，支持一键复制。
- 头部新增“模型参数”按钮，弹窗可配置 baseURL、API Key、模型名、temperature、max tokens、timeout、system prompt。
